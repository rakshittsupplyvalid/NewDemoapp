...
const [value, setValue] = useState(state?.form?.companyId || null);
const [items, setItems] = useState([]);

useEffect(() => {
  if (state?.fielddata?.companyid) {
    const dropdownItems = state.fielddata.companyid.map((x) => ({
      label: x.text,
      value: x.value,
    }));
    setItems(dropdownItems);
  }
}, [state?.fielddata?.companyid]);
  ...
  const handleSelectedCompany = (value) => {
    ...
    updateState({
      ...
      form: { 'DestinationBranch': companyName, companyId: value, branchvalue: '', DestinationDistrict: '' },
  ...
  const handleSelectedBranch = (value) => {
    ...
    updateState({
      ...
      form: { 'DestinationDistrict': branchname, branchvalue: value, districtid: '' },
      fielddata: { districtid: [] }
    });
    // alert(branchname)
    if (!value) return;
    let fielddata = { districtid: [] }
    const url = `/api/grouplocation`;
                ...
                <View style={styles.pickerContainer}>
                  ...
                  >
                    ...
                    {state?.fielddata?.districtid?.map((x) => (
                      <Picker.Item key={x.value} label={x.name} value={x.value} />
                    ))}
                  </Picker>
    ...
    api.get(url)
      .then((res) => {
        ...
        if (res?.data) {
          updateState({
            fielddata: {
              branchid: res.data.map((item) => ({
                id: item.id,
                name: item.name
              })),
              districtid: [] // Maintain the same structure
            }
          });
        }
      })
